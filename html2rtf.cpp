...
*/
static char OneChar(const char *in, int pos){
  char c = in[pos];
  if( c=='\n' ) linenum++;
  return c;
}
...
*/
void SgmlParse(
  const char *buffer            /* Read input from this buffer (must end with \0 !) */
){
  char c;               /* Character being parsed */ 
  int pos;             /* Actual position in input buffer */
  ...
  while( c ){
    if( c == ' ' ){
      ...
      */
      zBuf[0] = c;
      i = 1;
      while( i<sizeof(zBuf)-2 && (c=OneChar(buffer,pos++)) && c == ' ' ){
        zBuf[i++] = c;
      }
      zBuf[i] = 0;
      if( xSpaceHandler ){
      ...
      */
      int cQuote = 0;
      i = 0;
      while( (c=OneChar(buffer,pos++)) && (cQuote || c!='>') ){
      ...
      */
      ...
      for(j=0; zBuf[j] && zBuf[j] != ' '; j++){}
      if( zBuf[j] ){
        zBuf[j++] = 0;
        while( argc<(sizeof(argv)/sizeof(argv[0])) - 4 && zBuf[j] ){
          while( zBuf[j] == ' ' ) j++;
          argv[++argc] = &zBuf[j];
          while( zBuf[j] && zBuf[j] != ' ' && zBuf[j]!='=' ) j++;
          if( zBuf[j]!='=' ){
      ...
      */
      zBuf[0] = c;
      i = 1;
      while( i<sizeof(zBuf)-2 && (c=OneChar(buffer,pos++)) && c!='<' && c != ' '){
        zBuf[i++] = c;
      }
      zBuf[i] = 0;
      /* Dispose of a word */
...
void SgmlHandler(const char *zName, void (*xFunc)(int,const char**)){
  ...
  Handler *pNew = (Handler*)malloc( sizeof(Handler) + strlen(zName) + 1 );
  if( pNew==0 ) return;
...
*/
static void EmitRtfHeader(){
  static char zHeader[] =
    "{\\rtf1\\ansi"      /* This is an RTF document */
    "\\deff1"            /* The default font is f1 */
	"\\ansicpg1250"
    "\\deflang1045"      /* The language is U.S. English */
    "{\\fonttbl"         /* List of all fonts used */
       "{\\f1\\froman\\fcharset0\\fprq2 Tahoma;}"  /* Most text */
  ...
  if( inPreformat ){
    while( *zSpace ){
      switch( *zSpace ){
        case ' ':
        case '\t':
          if( !inPreformatStart ){ ret += ' '; }
          break;

        case '\n':
          if( !inPreformatStart ){ ret += "\\par\n"; }
          inPreformatStart = 0;
          break;
...
static void Preformat(int argc, const char **argv){
  ...
  char fsize[32] = {0}, isize[32] = {0};
  itoa((szFont-1)*2, fsize, 10);
  itoa(indent, isize, 10);
  ret += "{\\f3\\fs";
  ret += fsize;
  ret += "\\li";
  ret += isize;
...
static void BeginList(int argc, const char **argv){
  ...
  char unsize[32] = {0};
  char idepthsize[32] = {0};
  itoa(UNINDENT, unsize, 10);
  itoa(iDepth*INDENT, idepthsize, 10);
  LineBreak(0,0);
  ret += "{\\fi-";
  ret += unsize;
  ret += "\\li";
...
static void ListItem(int argc, const char **argv){
  ...
  if( iDepth>MAXDEPTH || iDepth<1 || aiCount[iDepth-1]==0 ){
	...
	char idepthsize[32] = {0};
    itoa(aiCount[iDepth-1]++, idepthsize, 10);
	ret += idepthsize;
	ret += ".\\tab ";
  }
  nSpace = 1;
...
static void BeginDefinitionList(int argc, const char **argv){
  ...
  LineBreak(0,0);
  char isize[32] = {0}, idepthsize[32] = {0};
  itoa(INDENT, isize, 10);
  itoa(iDepth*INDENT, idepthsize, 10);
  ret += "{\\fi-";
  ret += isize;
...
std::string html2rtf_parse(const char* in)
{
  ret.clear();
  EmitRtfHeader();
  SgmlParse(in);
